...
module apb_register #( 
    ...
    always_comb begin
        ...
        if (i_reg_rd_en) begin
            case (i_reg_addr)
                16'h0004: access_error = 1'b1; // WO read
            endcase
        end
        if (i_reg_wr_en) begin
            case (i_reg_addr)
                16'h0018: access_error = 1'b1; // RO write
            endcase
        end
    end
    ...
    always_ff @(posedge i_clk or negedge i_resetn) begin
        ...
        else if (i_reg_wr_en && addr_hit) begin
            case (i_reg_addr)
        ...
        else if (i_reg_rd_en && addr_hit) begin
    ...
    assign o_reg_error = (i_reg_rd_en || i_reg_wr_en) && (!addr_hit || access_error);
